...
void udpEchoTask(void)
{
   ...
   //Main loop
   {
      ...
      if(!error)
      {
    	  if (length > 3 && strncmp(context.buffer, "~~C", 3) == 0) {
    	      xil_printf("\r\nReceived command : ");
    	      xil_printf(" %c ", context.buffer[3]);

    	      const char *response = "Default";

    	      if (context.buffer[3] == 'O') {
    	          response = "Off";
    	      } else if (context.buffer[3] == 'N') {
    	          response = "Noise cancelling";
    	      } else if (context.buffer[3] == 'H') {
    	          response = "Hear through";
    	      }
    	      xil_printf("( %s ) \n\r", response);
