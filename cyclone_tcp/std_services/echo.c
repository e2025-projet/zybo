...
void handleSpecialMessage() {

	char_t message[] = "Hello UDP";

	if (strncmp(context.buffer, "BTNC", 4) == 0) {
		// Handle empty message
		print("Basys BNTC pressed\n\r");

	} else if (strncmp(context.buffer, "BTNU", 4) == 0) {
		// Handle ping
		print("Basys BNTU pressed\n\r");
	} else if (strncmp(context.buffer, "BTNR", 4) == 0) {
		// Handle ping
		print("Basys BNTR pressed\n\r");
	} else if (strncmp(context.buffer, "BTND", 4) == 0) {
		// Handle ping
		print("Basys BNTD pressed\n\r");
	} else if (strncmp(context.buffer, "BTNL", 4) == 0) {
		// Handle ping
		print("Basys BNTL pressed\n\r");
	}
}
   ...
   //Main loop
   {
      ...
      if(!error)
      {
         //Debug message
    	  ...
    	  const char_t *response = "Received message";
		...
		if (strncmp(context.buffer, "BTNC", 4) == 0) {
			...
			// Handle ping
			print("Basys BNTU pressed\n\r");
			response = "Bouton U wow c'est trop cool";
		} else if (strncmp(context.buffer, "BTNR", 4) == 0) {
			// Handle ping
			print("Basys BNTR pressed\n\r");
			response = "Bien recu ton bouton R, thanks Basys - avec amour, Zybo";
		} else if (strncmp(context.buffer, "BTND", 4) == 0) {
			// Handle ping
			print("Basys BNTD pressed\n\r");
			response = "BtnD? Ok je l'ai recu";
		} else if (strncmp(context.buffer, "BTNL", 4) == 0) {
			// Handle ping
			print("Basys BNTL pressed\n\r");
		}
         ...
         error = socketSendTo(context.socket, &ipAddr, port,
        		 response, strlen(response), NULL, 0);
      }
   }
}
