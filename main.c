...
void systemInit(void)
{
   ...
   if (error != NO_ERROR) {
       ...
       while(1) {
    	   xil_printf("netConfigInterface failed: %d\n\r", error);
...
void blinkTask()
{
	static const u8 blinkMsg[] = "Blink\n\r"; // or simply "B"
	if(counter++ > 5000)
	{
		// Toggle LED
		...
		XUartLite_Send(&UartLite, (u8*)blinkMsg, sizeof(blinkMsg) - 1); // Remove null terminator
...
void blinkTaskFast()
{
	static const u8 blinkMsg[] = "Fast blink\n\r"; // or simply "B"
	if(counter++ > 1000)
	{
		// Toggle LED
		XGpio_DiscreteWrite(&outputLED, 2, XGpio_DiscreteRead(&outputLED, 2) ^ 0x2);
